...
def reconcile_events_v2(
    ...
    output_path="c:/Users/Ashish Tiwari/OneDrive/Desktop/Ignite/ml/data/processed/canonical_incidents_v2.json"
):
    """
    Scale Event Reconciliation Engine (v2.0)
    Reconciles EM-DAT, DesInventar, IBTrACS, USGS, and GDACS into canonical incident entities.
    Avoids double-counting while linking multi-source records.
    """
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    
    canonical_incidents = []
    
    # 1. EM-DAT Historical Records
        ...
        with open(emdat_path, 'r', encoding='utf-8') as f:
            ...
            for rec in emdat_data:
                ...
                canonical_incidents.append({
                    ...
                    "matching_method": "OFFICIAL_EMDAT_PRIMARY"
                })

    # 2. DesInventar Subnational Records
    ...
    # 3. NOAA IBTrACS Tropical Cyclones
        ...
        with open(ibtracs_path, 'r', encoding='utf-8') as f:
            ...
            for rec in ibtracs_data:
                ...
                canonical_incidents.append({
                    ...
                    "latitude": None,
                    "longitude": None,
                    "source_records": [{"source": "NOAA_IBTrACS", "source_id": rec['sid']}],
                    "confidence_score": 0.99,
                    "matching_method": "NOAA_BEST_TRACK"
                })

    # 4. Multi-Year USGS Historical Earthquakes (Filtered for Mw >= 5.5)
    if os.path.exists(usgs_path):
        with open(usgs_path, 'r', encoding='utf-8') as f:
            usgs_data = json.load(f)
            features = usgs_data.get('features', [])
            # Deduplicate & ingest major seismic events
            for feat in features:
                props = feat.get('properties', {})
                mag = props.get('mag', 0)
                if mag >= 6.0:  # Select major historical earthquakes for canonical incidents
    ...
    print(f"Successfully reconciled {len(canonical_incidents)} canonical incidents v2.0 to {output_path}")
