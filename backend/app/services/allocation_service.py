...
class AllocationService:
    ...
    def optimize(self, request: OptimizationRequest) -> OptimizationResponse:
            ...
            if latest and latest.priority_score is not None:
                priority_assessments.append(
                    PriorityAssessment(
                        ...
                        explanation=f"Fetched from latest assessment {latest.assessment_id}"
                    )
                )
            else:
                # Fallback to zero priority if no assessment found
                priority_assessments.append(
                    PriorityAssessment(
                        verified_incident_id=inc_id,
        ...
        for inc_id in request.incident_ids:
            ...
            for n in needs:
                # Qualitative needs (quantity is None) are ignored by the optimizer naturally.
                # But we map them correctly anyway according to schemas.
        ...
        # 6. Map AllocationResult edges into internal records
        records = []
